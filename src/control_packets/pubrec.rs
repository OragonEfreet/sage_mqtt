...
impl PubRec {
    /// AsyncWrite the `PubRec` body of a packet, returning the written size in bytes
    /// in case of success.
    ...
    /// AsyncRead the `PubRec` body from `reader`, retuning it in case of success.
    pub async fn read<R: AsyncRead + Unpin>(reader: &mut R, shortened: bool) -> SageResult<Self> {
        ...
        if shortened {
            ...
            pubrec.reason_code =
                ReasonCode::try_parse(codec::read_byte(reader).await?, ControlPacketType::PUBREC)?;
